...
We use a set of translations (of a corpus) to refer to a collection that includes one translation per each source sentence in the corpus. The goal is to assemble one set of high quality translations while minimizing the associated cost. 
...
from the  2009 NIST Urdu-English Open Machine Translation Evaluation set\footnote{LDC Catalog number LDC2010T23}. A total of 51 [or 52??] Turkers contributed translations. Turkers also filled out a survey about their language skills and their countries of origin. Each Urdu sentence was translated by 4 non-professional Turkers and 4 professional translators hired by the LDC. The cost of translating a sentence is \$0.10 by non-professional and \$6.03 (\$0.30 per word $\times$ 20.1 words per sentence on average) by professional translators. 

Following \newcite{zaidan-callisonburch:2011:ACL-HLT2011a}, we use the BLEU score \cite{papineni2002bleu} against all 4 professional translations as a gold-standard to measure the quality of a non-professional translation. While a set of professional translations has a quality of 42.38 BLEU score, a Turker set only scores 28.13 BLEU on average. \newcite{zaidan-callisonburch:2011:ACL-HLT2011a} trained a MERT model to select one non-professional translation out of the four and pushed the quality of one set of crowdsourcing translation to 39.06, on par of professional translations. They used a small amount 10\% of professional translations as calibration data to estimate the goodness of the non-professional translation. In total, their approach needs 4 non-professional translations for each source sentence and 4 different professional translations for about 180 sentences to assemble one set of good translations.  

However, very large parallel corpora, often millions of pairs of sentences, are required in training statistical machine translation systems. It is still costly and even difficult to recruit enough numbers of both professional and non-professional translators for a low-resource language or in a short time. In this paper, we study how to reduce the number of non-professional translations and the number of professional translations needed to assemble one set of translations of good quality. 
...
We introduce a new bilingual feature in addition to the feature set used by \newcite{zaidan-callisonburch:2011:ACL-HLT2011a} in their MERT model. All features are listed below:
...
\item Ranking features: 3 features including translation's average ranking and the percentages of being evaluated better and best. [UNCLEAR, NEED TO REVISE]
...
The first mechanism to optimize cost is recognizing good translations to stop purchasing more translations immediately after. 
