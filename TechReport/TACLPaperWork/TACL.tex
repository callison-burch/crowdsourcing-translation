...
\newcite{lin2014re} discussed what makes classifiers performs well when dealing with relabeled annotation data. 

\section{Discussion}
\begin{figure*}[htbp]
  \centering
  \includegraphics[width=\linewidth]{cost-bleu/pricecost.pdf}
  \caption{The Relationship between BLEU score and costs. Figure (a) shows the relationship between BLEU score and  the total costs (professional and non-professional) for the ranking based approach. Figure (b) shows the relationship between BLEU score and the total costs for model-based approach. Figure (c)  illustrates the relationship between BLEU score and non-professional costs for model based approach.}
    \label{fbleucost}
\end{figure*}

Figure \ref{fbleucost} details the relationship between BLEU score and costs.  From Figure \ref{fbleucost}-(a), in the ranking selection approach, the curve illustrates that after the costs increases to some point, the BLEU score stopped increasing even though we invest more money to buy professional translations as workers' ranking calibrations continually. This means  we can obtain a high ranking accuracy compared with gold standard ranking and a relative high BLEU score in ranking selection approach by paying only a small amount of money to buy a few professional translations.  

Figure\ref{fbleucost}-(b) shows that in model based selection method, as \textit{delta} increases, the cost increases, and the BLEU score for translations selected by model goes higher. Since the cost for purchasing professional translations is fixed and takes a major proportion in the total cost for  this approach, cost's increment which is contributed by the investment on non-professional translations is not very big as \textit{delta} increase. Thus, to emphasize the effectiveness of model selection approach, we figure the relationship between BLEU score and non-professional cost in figure \ref{fbleucost}-(c). The total amount money for buying  complete 4 non-professional translation set for source sentences in testing set  is \$573.44 ($716.80 \times 0.80$). Table \ref{orderanother} shows that  $S_{upper}$ is 40.13. If we pay \$286.72, which is \%50 of total non-professional translations, the translating accuracy is as  \%94 as $S_{upper}$ and the BLEU score is 37.61.  Furthermore, suppose we pay \$348.36, which is \%60.7 of total non-professional translations, the translating accuracy increases to  \%96 of $S_{upper}$ and the BLEU score becomes 38.52. 
