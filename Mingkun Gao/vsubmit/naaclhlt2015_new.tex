...
We rank the Turkers using the first few translations that they provide by comparing their translations to the professional translations of those sentences. Ranking workers on gold standard data would allow us to discard bad workers.
This is similar to the idea of a qualification test in MTurk. 
...
\subsubsection{Baseline and gold standard}
We evaluate ranking quality in weighted correlation($\rho$) compared with the gold standard ranking of workers. 
To establish the gold standard ranking, we score each Turker based on the average BLEU score of all his/her translations against professional references.

To establish a baseline ranking, we use the MERT model \cite{och2003minimum} that was employed by \newcite{zaidan-callisonburch:2011:ACL-HLT2011a}. The MERT model's ranking of translators achieves a correlation of 0.74 against the gold standard ranking. 
...
The  weighted correlation is 0.91.  }
    \label{first2hitrank}
\end{figure}
%
\begin{figure}
...
Table \ref{lrresult} shows that the model trained on all features achieves a very high correlation with the gold standard ranking (Spearman's $\rho$ = 0.95), and a  BLEU score of 39.80.
...
Top(\%) &random & model & gold & $\Delta$ & \# Trans\\\hhline{======}
